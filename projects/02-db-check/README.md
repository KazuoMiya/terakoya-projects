...
**主線は PostgreSQL。Oracle の人は、まず PostgreSQL 版を作る。そのうえで、
...
> 項目10が最初から何かを報告してくる。それは**その項目が生きている証拠**だ。
...
一度に10項目を狙わない。各段で「動くもの」を持ちながら進む。

1. **環境と繋がる** — `docker compose up -d` → healthy を待つ → psql で `\dt` が見える
2. **純関数を緑に** — `mask_dsn` / `judge_ratio` / `diff_snapshot` を書いて、採点テストを緑にする（DBはまだ触らない）
3. **項目1だけの点検** — 接続して `SELECT 1` の応答時間を測り、結果を1行表示する。**ここで初めてDBに繋がったツールになる**
4. **10項目へ広げる** — 1項目ずつ足す。足すたびに実行して、表示が増えるのを見る
5. **仕上げ** — JSON保存と前日差分、`--json`、ログのマスク確認。expected-output.md と見比べる

## 異常を作って、検知できることを確かめる

**点検ツールは「異常を見つけられて初めて、点検ツール」だ。**
正常なDBに対して「全部OK」が出ても、それは何も証明していない。
別のターミナルをもう1枚開いて、わざと異常を作ってみよう。
...
この状態で `python src/db_check.py` を実行し、**項目8が拾えているか**を確かめよう。
