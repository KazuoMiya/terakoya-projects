...
> 覗けないからだ。修了の重みは、完了チェックリストと、あなたの手順書が担う。
...
- **Mac の人**: Mac に systemd は無い。**Multipass**（Ubuntu の開発元 Canonical 公式の
  ...
  ```
  インストールは **canonical.com/multipass（公式サイト）のインストーラ**から（macOS 14以降・Apple Silicon対応・無料）。
...
**反映の前に必ず `sudo nginx -t`**——文法ミスのまま reload して玄関を壊さないためだ）。
...
コメントを外し、鍵と証明書を作り、**`sudo nginx -t` を通してから
`sudo systemctl reload nginx`**（設定を変えたら毎回この2つだ）。`https://localhost/health` を叩く
